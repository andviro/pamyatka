...
механизм "тайников". Пусть в рабочем дереве ветки `develop` есть несохраненные изменения. Для их
сохранения нужно выполнить команду `git stash save` или просто `git stash`, в результате которой
индекс и изменения в рабочем дереве будут сохранены в специальном хранилище-тайнике, а дерево
проекта и индекс окажутся сброшенными в вершину ветки. После этого можно будет свободно сменить
ветку и продолжить работу в ней. Если изменения потребуется вернуть из тайника, достаточно выполнить
команду `git stash apply`. Причем, сохранить изменения можно в одной ветке, а применить их
