...
исполняемые модули, файлы восстановления. Список шаблонов файлов и директорий,
которые следует игнорировать, можно задать в файле `.gitignore`. Пример файла,
в котором задается игнорирование скомпилированных скриптов на Python и
содержимого директории `temp` приведен ниже:
...
коммит, необязательно разделять процесс на добавление в индекс и собственно
коммит. Команда `git commit -a` автоматически добавляет в индекс все измененные
файлы и предлагает ввести сообщение в журнал. Команду следуюет использовать с
осторожностью, так как в коммит могут нечаянно попасть изменения, которые
планировалось сохранить в следующем коммите.
...
в состояние последнего сохраненного коммита. 
...
При этом не указываются ключи `-a` и `-m`.
...
* Отмените изменения одного из файлов
